...
router.post('/discuss', async function (req, res) {
...
router.get('/discuss', async function (req, res) {
  const model = await Discuss.find({ productId: req.query.productId }).populate('Shops')
  res.send(model)
})
